...
def limitar_por_red():
    if request.endpoint == "static":
        return
    # Las clases online no requieren estar en la red del hotspot
...
USERS = {
    ...
    "ana.rios@ulagos.cl":   {"password": "1234", "rol": "estudiante"},
    "pedro.vega@ulagos.cl": {"password": "1234", "rol": "estudiante"},
}
...
def forgot_password():
    message = "Enlace de recuperacion enviado a su correo institucional."
...
def panel():
    ...
    return render_template(
        ...
        nombre_clase="Programacion Web",
        rol=session["rol"],
    )


# GENERAR QR — presencial (GET) y online (POST con codigo_clase)
...
def generar_qr():
    ...
    if request.method == "POST":
        # Modalidad online: el docente envia el codigo corto desde el formulario
        codigo_clase = request.form.get("codigo_clase", "").strip().upper()
        if not codigo_clase:
            return redirect(url_for("panel"))
        modalidad = "online"
    else:
        # Modalidad presencial: GET directo como siempre
        codigo_clase = None
...
def asistir(codigo):
    ...
    if session["rol"] != "estudiante":
        return render_template("resultado_asistencia.html",
                               mensaje="Esta accion es solo para estudiantes.",
                               color="red",
                               usuario=session["email"],
                               rol=session["rol"])
    ...
    if not qr_encontrado:
        return render_template("resultado_asistencia.html",
                               mensaje="El codigo QR no es valido o ya expiro.",
                               color="red",
                               usuario=email_est,
                               rol=session["rol"])
...
def asistir_online(codigo):
    # Esta ruta no requiere validacion por red (ver before_request)
    if "email" not in session:
        session["codigo_pendiente"] = codigo
    ...
    if request.method == "POST":
        ...
        if codigo_ingresado == qr_encontrado.get("codigo_clase", ""):
            ...
            return render_template("codigo_online.html",
                                   ...
                                   usuario=email_est,
                                   rol=session["rol"],
                                   error="Codigo incorrecto. Intentalo de nuevo.")

    # GET: mostrar el formulario para ingresar el codigo de clase
    return render_template("codigo_online.html",
                           codigo=codigo,
                           usuario=email_est,
                           rol=session["rol"],
